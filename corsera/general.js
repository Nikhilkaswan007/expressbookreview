...
public_users.get('/author/:author',function (req, res) {
  ...
  let get_books = new Promise((resolve, reject) => {
    try {
      // Filter books by author (case-sensitive exact match)
      ...
      reject(new Error(`Error processing request: ${error.message}`));
    }
  });
  ...
  get_books.then((filtered_books) => {
    ...
    if (err.message.includes('No books found')) {
      res.status(404).json({message: err.message});
    } else {
...
public_users.get('/title/:title',function (req, res) {
  ...
  let get_books = new Promise((resolve, reject) => {
    try {
      // Filter books by title (case-sensitive exact match)
